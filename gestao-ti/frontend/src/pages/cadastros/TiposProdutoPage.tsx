...
import { Plus, Tag, Pencil, Check, X, ArrowUpDown, ArrowUp, ArrowDown, Trash2, Search } from 'lucide-react';
...
export function TiposProdutoPage() {
  ...
  async function carregar() {
    setLoading(true);
    try { setTipos(await compraService.listarTiposProduto()); } catch { /* empty */ }
    setLoading(false);
  }
  ...
  const sorted = useMemo(() => {
    ...
    const base = termo
      ? tipos.filter((x) => [x.codigo, x.descricao].some((v) => (v ?? '').toString().toLowerCase().includes(termo)))
      : tipos;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      await compraService.criarTipoProduto({ codigo, descricao });
      setCodigo(''); setDescricao('');
      setShowForm(false);
      toast('success', 'Tipo de produto criado');
      carregar();
    } catch { toast('error', 'Erro ao criar tipo de produto'); }
    setSaving(false);
  }

  function startEdit(t: TipoProduto) {
  ...
  async function saveEdit() {
    ...
    setEditSaving(true);
    try {
      await compraService.atualizarTipoProduto(editId, { codigo: editCodigo, descricao: editDescricao });
  ...
  async function handleToggleStatus(t: TipoProduto) {
    const novoStatus = t.status === 'ATIVO' ? 'INATIVO' : 'ATIVO';
    try {
      await compraService.atualizarTipoProduto(t.id, { status: novoStatus });
  ...
  return (
    <>
      <Header title="Tipos de Produto" />
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          <p className="text-sm text-slate-500">Classificacao de produtos (Periferico, Computador, Componente, Rede, etc.)</p>
          {canManage && (
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <input type="text" value={codigo} onChange={(e) => setCodigo(e.target.value)} required maxLength={10}
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-capul-600" placeholder="PERIF" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Descricao *</label>
                <input type="text" value={descricao} onChange={(e) => setDescricao(e.target.value)} required maxLength={100}
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-capul-600" placeholder="Periferico" />
              </div>
            </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-slate-500">Nenhum tipo de produto cadastrado</p>
          </div>
        ) : (
                    ...
                    {editId === t.id ? (
                      <>
                        <td className="px-6 py-3">
                          <input value={editCodigo} onChange={(e) => setEditCodigo(e.target.value)} maxLength={10}
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              ...
                              <button onClick={async () => {
                                if (!await confirm('Excluir Tipo de Produto', `Excluir tipo "${t.descricao}"?`, { variant: 'danger' })) return;
                                try { await compraService.excluirTipoProduto(t.id); carregar(); toast('success', 'Tipo excluido'); }

...
import { Plus, Wallet, Pencil, Check, X, Trash2, ArrowUpDown, ArrowUp, ArrowDown, Search } from 'lucide-react';
...
export function CentrosCustoPage() {
  ...
  const sorted = useMemo(() => {
    ...
    const base = termo
      ? centrosCusto.filter((cc) => [cc.codigo, cc.nome, cc.status].some((v) => (v ?? '').toString().toLowerCase().includes(termo)))
      : centrosCusto;
  ...
  }, [centrosCusto, busca, sortKey, sortDir]);

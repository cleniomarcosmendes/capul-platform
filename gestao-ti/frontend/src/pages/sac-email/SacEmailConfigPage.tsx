...
/**
 * SAC — Configuração de e-mail de entrada. Só configuração (conexão IMAP +
 * operação do poller). Admin/Gestor. O trabalho do dia a dia (buscar/triagem)
 * fica na tela "SAC — Triagem".
 */
          ...
          {loading ? (
            ...
            <>
              <p className="text-sm text-slate-500">
                Configuração do poller IMAP do e-mail de <strong>entrada</strong> do SAC. O trabalho do dia a dia
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-5">
                ...
                <div className="mt-4 flex items-center gap-3">
                  ...
                  {teste && (
                    <span className={`inline-flex items-center gap-1.5 text-sm ${teste.ok ? 'text-emerald-700' : 'text-red-600'}`}>
                      {teste.ok ? <CheckCircle2 className="w-4 h-4" /> : <XCircle className="w-4 h-4" />}
                      {teste.ok ? `${teste.mailbox}: ${teste.total} mensagem(ns), ${teste.unseen} não lida(s)` : teste.error}
                    </span>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  {(() => {
                    ...
                    const cls = ativo ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200';
                ...
                <Toggle label="Consumo automático ligado" hint="Liga o poller agendado (busca os e-mails no intervalo abaixo)." checked={enabled} onChange={setEnabled} />
                <Toggle label="Pausar (freio de mão)" hint="Pausa só o consumo AUTOMÁTICO (poller). Não afeta o “Buscar agora” manual da Triagem." checked={pauseSync} onChange={setPauseSync} />

                <div className="grid grid-cols-2 gap-4">
                  <label className="block">
                    <span className="text-xs text-slate-500">Pasta IMAP</span>
                    <input value={mailboxFolder} onChange={(e) => setMailboxFolder(e.target.value)}
                  ...
                  <label className="block">
                    ...
                    <input type="number" min={1} max={1440} value={pollIntervalMinutes} onChange={(e) => setPollIntervalMinutes(Number(e.target.value))}
                      className="mt-1 w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-capul-600" />
                  </label>
                </div>

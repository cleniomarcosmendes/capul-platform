...
- **Passos:** (1) gerar uma despesa PENDENTE de VEÍCULO: no detalhe de uma **saída de veículo** do depto dela (ex.: rota #10, veículo KELVER = FBR), **Lançar despesa** com um tipo de **categoria VEÍCULO** — ⚠️ **use Combustível / Manutenção / Pedágio (NÃO "Alimentação"**, que é INDIVÍDUO e vira sem-veículo → gestor-only). (2) Em **Custos da Frota** (como lidyanerocha) filtrar Pendentes → a despesa aparece → **Aprovar**. *(O botão "Lançar despesa" direto em Custos da Frota nasce APROVADA — não serve p/ testar aprovação; o PENDENTE vem da saída de veículo.)*

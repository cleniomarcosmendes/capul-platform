// ⚠️ SUPERADO em 05/09/2026 — este arquivo foi o INSUMO do módulo.
// A versão viva é gestao-pessoas/backend/prisma/seed.ts (adaptada ao catálogo
// rh.criterio). Editar aqui não tem efeito nenhum e cria a segunda cópia da
// regra, que envelhece errada. Mantido só como registro do que foi entregue.
...
//
// ATENÇÃO — o que aqui é DECISÃO DA ARIELLY e está apenas pré-preenchido:
...
type DefGrupoAuto = {
  titulo: string;
  tipo: 'ESCOLARIDADE' | 'TEMPO_EMPRESA' | 'TEMPO_FUNCAO' | 'QTDE_TREINAMENTO';
  peso: number;
  faixas: unknown[];
...
const AUTOMATICOS = (pesos: [number, number, number, number]): DefGrupoAuto[] => [
  { titulo: 'Escolaridade',           tipo: 'ESCOLARIDADE',     peso: pesos[0], faixas: FAIXAS_ESCOLARIDADE, faixaTipo: 'DOMINIO'  },
  { titulo: 'Tempo de Empresa',       tipo: 'TEMPO_EMPRESA',    peso: pesos[1], faixas: FAIXAS_TEMPO_EMPRESA, faixaTipo: 'NUMERICA' },
  { titulo: 'Tempo na Função',        tipo: 'TEMPO_FUNCAO',     peso: pesos[2], faixas: FAIXAS_TEMPO_FUNCAO,  faixaTipo: 'NUMERICA' },
  { titulo: 'Treinamentos no Período',tipo: 'QTDE_TREINAMENTO', peso: pesos[3], faixas: FAIXAS_TREINAMENTO,   faixaTipo: 'NUMERICA' },
];

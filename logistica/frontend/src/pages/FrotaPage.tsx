...
function SaidaForm({ veiculos, onDone }: { veiculos: VeiculoDisp[]; onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    if (!mat) { setRdvCandidatos([]); setRdvSel(''); return; }
    logisticaApi.get<{ id: string; numero: number; supervisorNome: string | null }[]>('/frota/rdv-candidatos', { params: { matricula: mat } })
      .then((r) => { setRdvCandidatos(r.data); setRdvSel(r.data[0]?.id ?? ''); })
      ...
      <div className="space-y-6">
        ...
        <div className={podeAvancar ? '' : 'opacity-60'}>
          ...
          <div className="grid grid-cols-1 gap-x-4 gap-y-3 sm:grid-cols-12">
            ...
            {rdvCandidatos.length > 0 && (
              <div className="sm:col-span-12">
                <label className="mb-1 block text-sm font-medium text-slate-600">Vincular ao RDV (planejamento do supervisor)</label>
                <select
                  value={rdvSel} onChange={(e) => setRdvSel(e.target.value)} disabled={!podeAvancar}
                  className="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-base disabled:bg-slate-100"
                >
                  <option value="">— Não vincular —</option>
                  {rdvCandidatos.map((r) => (
                    <option key={r.id} value={r.id}>RDV #{r.numero}{r.supervisorNome ? ` · ${r.supervisorNome}` : ''}</option>
                  ))}
                </select>
                <p className="mt-1 text-xs text-emerald-600">O KM desta saída entra no RDV do supervisor (sugerido automaticamente pela matrícula).</p>
              </div>
            )}
